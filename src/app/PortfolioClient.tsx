    ...
    <div ref={containerRef} className="relative">
      ...
      <section className="section-brutal min-h-screen flex flex-col justify-between relative overflow-hidden hero-dark">
        ...
        <nav className="flex flex-col gap-2 absolute top-8 left-8 md:left-12 z-30">
          ...
          <a href="/Kiran_Kushal_CV.pdf" download className="nav-link nav-item flex items-center gap-1 text-[#c8956c] opacity-80 hover:opacity-100 transition-opacity mt-4">
        ...
        {/* Ink blot images — scattered asymmetrically */}
        <div className="hero-ink ink-blot absolute top-[8%] right-[12%] w-[22vw] max-w-[320px] opacity-60 rotate-3 z-10">
          <img src="/projects/fk-sankranthi.jpg" alt="" className="w-full h-auto" />
        </div>
        <div className="hero-ink ink-blot absolute top-[2%] right-[0%] w-[16vw] max-w-[250px] opacity-40 -rotate-6 z-10">
          <img src="/projects/content-strategy.jpg" alt="" className="w-full h-auto" />
        </div>
        <div className="hero-ink ink-blot absolute bottom-[40%] right-[6%] w-[18vw] max-w-[270px] opacity-50 rotate-2 z-10">
          <img src="/projects/mastaru-gari-logo.jpg" alt="" className="w-full h-auto" />
        </div>
        ...
        <div className="flex flex-col flex-grow relative z-20 mt-20 md:mt-0 pt-16 md:pt-24">
          ...
          <div className="flex flex-col md:flex-row gap-8 md:gap-16 items-start">
            <div className="hero-bio-column md:ml-[8vw]" style={{ fontStyle: "italic" }}>
              {bioWords.map((word, i) => (
      ...
      <section className="py-8 border-y border-[var(--color-ink-faint)]/10 overflow-hidden bg-[var(--color-paper)]">
        <div className="marquee-container w-full h-[60px] md:h-[80px]">
          ...
          <div className="flex flex-wrap gap-4 mt-8">
            {categories.map((category) => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-xs md:text-sm tracking-widest uppercase transition-colors duration-300 font-mono ${
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2 text-left">
              <label htmlFor="name" className="text-xs font-mono uppercase tracking-widest text-[var(--color-ink-faint)]">Name</label>
              <input type="text" id="name" name="name" required className="bg-transparent border-b border-[var(--color-ink-faint)]/30 py-3 outline-none text-[var(--color-ink)] focus:border-[#c8956c] transition-colors" placeholder="John Doe" />
            </div>
            <div className="flex flex-col gap-2 text-left">
              <label htmlFor="email" className="text-xs font-mono uppercase tracking-widest text-[var(--color-ink-faint)]">Email</label>
              <input type="email" id="email" name="email" required className="bg-transparent border-b border-[var(--color-ink-faint)]/30 py-3 outline-none text-[var(--color-ink)] focus:border-[#c8956c] transition-colors" placeholder="john@example.com" />
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2 text-left">
              <label htmlFor="serviceRequested" className="text-xs font-mono uppercase tracking-widest text-[var(--color-ink-faint)]">Service</label>
              <select id="serviceRequested" name="serviceRequested" required className="bg-transparent border-b border-[var(--color-ink-faint)]/30 py-3 outline-none text-[var(--color-ink)] focus:border-[#c8956c] transition-colors appearance-none cursor-pointer rounded-none">
                <option value="Branding" className="bg-[var(--color-paper)]">Branding</option>
                <option value="Packaging" className="bg-[var(--color-paper)]">Packaging</option>
                <option value="Social Media" className="bg-[var(--color-paper)]">Social Media</option>
                <option value="Print Design" className="bg-[var(--color-paper)]">Print Design</option>
              </select>
            </div>
            <div className="flex flex-col gap-2 text-left">
              <label htmlFor="budget" className="text-xs font-mono uppercase tracking-widest text-[var(--color-ink-faint)]">Budget</label>
              <select id="budget" name="budget" required className="bg-transparent border-b border-[var(--color-ink-faint)]/30 py-3 outline-none text-[var(--color-ink)] focus:border-[#c8956c] transition-colors appearance-none cursor-pointer rounded-none">
                <option value="< $1k" className="bg-[var(--color-paper)]">&lt; $1k</option>
                <option value="$1k - $5k" className="bg-[var(--color-paper)]">$1k - $5k</option>
                <option value="$5k+" className="bg-[var(--color-paper)]">$5k+</option>
              </select>
            </div>
          </div>

          <div className="flex flex-col gap-2 text-left mt-4">
            <label htmlFor="message" className="text-xs font-mono uppercase tracking-widest text-[var(--color-ink-faint)]">Project Details</label>
            <textarea id="message" name="message" required rows={4} className="bg-transparent border-b border-[var(--color-ink-faint)]/30 py-3 outline-none text-[var(--color-ink)] focus:border-[#c8956c] transition-colors resize-none" placeholder="Tell me about your project..."></textarea>
          </div>

          <button type="submit" className="mt-8 px-8 py-4 bg-[var(--color-ink)] text-[var(--color-paper)] font-mono text-sm tracking-widest uppercase hover:bg-[#c8956c] hover:text-[var(--color-ink)] transition-colors duration-300 w-full md:w-auto self-center">
            Send Inquiry
          </button>
        </form>

        <div className="flex justify-center gap-8 mt-24">
          <a href="#" className="nav-link inline-block cmyk-shift">Behance</a>
          <a href="#" className="nav-link inline-block cmyk-shift">LinkedIn</a>
          <a href="#" className="nav-link inline-block cmyk-shift">Dribbble</a>
          <a href="#" className="nav-link inline-block cmyk-shift">Instagram</a>
        </div>
      </section>
                  ...
                  <div className="flex gap-4 border-b border-white/10 mb-8 overflow-x-auto">
                    {["Overview", "Metrics"].map((tab) => (
                      <button
                        ...
                        className={`pb-2 text-xs whitespace-nowrap transition-colors duration-300 font-mono uppercase tracking-widest border-b-2 ${
                          modalTab === tab 
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        <AnimatePresence>
          {isChatOpen && (
            <motion.div
              ...
              className="w-80 md:w-96 h-[500px] mb-4 bg-[#0a0a0a] border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
            >
              <div className="p-4 border-b border-white/10 bg-white/5 backdrop-blur-md flex justify-between items-center">
                <span className="font-mono text-xs uppercase tracking-widest text-white">AI Assistant</span>
                <button onClick={() => setIsChatOpen(false)} className="text-white/50 hover:text-white">✕</button>
              </div>
              
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
                {messages.length === 0 && (
                  <div className="text-white/50 text-xs font-mono text-center my-auto">
                    Ask me anything about Kiran's experience, tech stack, or portfolio projects.
                  </div>
                )}
                {messages.map((m: any) => (
                  <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-lg p-3 text-sm font-body ${
                ...
                {isLoading && (
                  <div className="flex justify-start">
                    <div className="bg-white/10 text-white rounded-lg p-3 text-sm font-body animate-pulse">
                      Thinking...
                    </div>
                  </div>
                )}
              ...
              <form onSubmit={handleSubmit} className="p-4 border-t border-white/10 bg-white/5 backdrop-blur-md flex gap-2">
                <input
                  className="flex-1 bg-transparent border border-white/20 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-white/50 font-body placeholder:text-white/30"
                  value={input}
                ...
                <button 
                  ...
                  className="bg-white text-black px-4 py-2 rounded-lg font-mono text-xs uppercase tracking-widest disabled:opacity-50"
                >
                  Send
                </button>
              </form>
        ...
        <button
          ...
          className="w-14 h-14 rounded-full bg-white text-black shadow-2xl flex items-center justify-center hover:scale-105 transition-transform"
        >
          {isChatOpen ? '✕' : <span className="font-mono text-xs font-bold tracking-tighter">AI</span>}
        </button>
      </div>
